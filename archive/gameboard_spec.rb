...
describe Gameboard do
  ...
  it 'assigns an y-coordinate to a gamespace' do
    ...
    expect(new_gameboard.grid[1].spaces_array[0].y_coord).to eq(1)
  end

  it 'counts the number of adjacent mines for each space' do
    new_gameboard = Gameboard.new(3, 3, 9)
    new_gameboard.generate_grid
    new_gameboard.place_pieces
    ...
    expect(new_gameboard.user_move(row, col).adjacent_mines).to eq(3)
  end
